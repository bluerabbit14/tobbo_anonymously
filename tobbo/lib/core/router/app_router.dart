...
GoRouter createAppRouter(SessionRepositoryImpl sessions) {
  return GoRouter(
    initialLocation: sessions.hasStoredIdentity ? AppRoutes.home : AppRoutes.welcome,
    refreshListenable: sessions,
    redirect: (context, state) {
      final hasIdentity = sessions.hasStoredIdentity;
      final onWelcome = state.matchedLocation == AppRoutes.welcome;
      if (!hasIdentity && !onWelcome) return AppRoutes.welcome;
      if (hasIdentity && onWelcome) return AppRoutes.home;
      return null;
    },
    routes: [
      GoRoute(
